...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{tcolorbox}[colback=layer1, title=\textbf{降本增效（Efficiency）}]
        \small
        \textbf{成本控制}
        \begin{itemize}
          \item \textbf{Token 节约 30\%+}：通过图谱精准剪枝，无需将无关代码塞入 Context
          \item \textbf{解决率提升 20-30\%}：避免 Agent 在错误路径上反复尝试（Loop），直接阻断已知错误分支
        \end{itemize}
    ...
    \begin{column}{0.48\textwidth}
      \begin{tcolorbox}[colback=layer2, title=\textbf{资产沉淀（Asset）}]
        \small
        \textbf{知识资产化}
        \begin{itemize}
          \item \textbf{项目活文档}：图谱实时反映代码架构，替代过时的开发文档
          \item \textbf{经验数字化}：将工程师的 Debug 经验结构化存储，新人/Agent 可直接复用
        \end{itemize}
        ...
        \textbf{技术壁垒}
        \begin{itemize}
          \item 图谱随项目成长，形成\textcolor{dotred}{不可复制的私有资产}
          \item 数据飞轮效应：越用越聪明
        \end{itemize}
      \end{tcolorbox}
    \end{column}
  ...
  \begin{center}
    \Large
    \textbf{最终目标：} 从"辅助写代码"进化为 \textcolor{accentblue}{\textbf{"懂业务、有记忆、能进化"}} 的智能研发伙伴
  \end{center}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{节点类型设计}
        ...
        \begin{itemize}
          \item \texttt{rule}: "遇到此类报错，通常需修改 A 配置"
          \item \texttt{confidence}: 验证成功率（如 0.85）
          \item \texttt{source\_tasks}: 来源任务，可追溯
          \item \texttt{applicable\_context}: 适用场景
        \end{itemize}
        \vspace{0.1cm}
        \textcolor{darkgreen}{\textbf{业务价值}}：将"老司机经验"显式化
      \end{block}
    \end{column}
