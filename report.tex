...
\begin{frame}
  ...
  \begin{tikzpicture}[node distance=0.8cm, every node/.style={align=center}]
    % 节点样式
    \tikzstyle{level} = [rectangle, rounded corners, minimum width=2.8cm, minimum height=1.2cm, draw=black, font=\small]

    % LLM
    \node[level, fill=gray!20] (llm) at (0, 0) {\textbf{LLM}\\理解语言、生成代码};
    \node[right=0.3cm of llm, font=\scriptsize, text=red] {无法与外界交互\\无记忆、一次性输出};

    % Agent
    \node[level, fill=blue!20, below=0.6cm of llm] (agent) {\textbf{Agent = LLM + Tools}\\读文件、执行命令};
    \node[right=0.3cm of agent, font=\scriptsize, text=red] {没有记忆\\每次从零开始，重复犯错};

    % Agent + Memory
    \node[level, fill=green!20, below=0.6cm of agent] (memory) {\textbf{Agent + Memory}\\存储和检索历史经验};
    \node[right=0.3cm of memory, font=\scriptsize, text=red] {扁平存储，无法表达关系\\检索不精准，无法推理};

    % Agent + Context Graph
    \node[level, fill=orange!30, below=0.6cm of memory] (cg) {\textbf{Agent + Context Graph}\\结构化存储、路径推理};
    \node[right=0.3cm of cg, font=\scriptsize, text=darkgreen] {\textbf{我们要研究的方向}};

    % 箭头
    \draw[->, thick] (llm) -- node[left, font=\scriptsize] {+工具} (agent);
    \draw[->, thick] (agent) -- node[left, font=\scriptsize] {+记忆} (memory);
    \draw[->, thick] (memory) -- node[left, font=\scriptsize] {+图结构} (cg);
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}
  \frametitle{从"存储数据"到"记录决策"}

  \begin{block}{传统系统的局限}
    CRM、ERP 等系统只记录\textbf{最终状态}：一笔订单以 40\% 折扣成交
    \begin{itemize}
      \item 但不记录：VP \textbf{为什么}批准这个折扣？基于什么先例？
      \item 决策过程散落在 Slack、会议记录、邮件中，随员工离职而消失
    \end{itemize}
  \end{block}

  \begin{block}{AI Agent 的记忆需求}
    \begin{itemize}
      \item Agent = LLM + 工具调用 + 循环执行
      \item 复杂任务需要 Agent 记住：过去的尝试、失败原因、成功策略
      \item \textbf{核心问题}：如何有效组织和检索 Agent 的历史经验？
    \end{itemize}
  \end{block}

  \begin{alertblock}{两个平行的需求}
    \textbf{企业侧}：捕获人类决策过程 $\longleftrightarrow$ \textbf{Agent 侧}：组织 Agent 执行轨迹
  \end{alertblock}
\end{frame}
...
\begin{frame}
  ...
  \begin{tabular}{p{2.2cm}|p{4.2cm}|p{5.5cm}}
    ...
    Neural: 黑盒，无法解释\\
    & & Graph: "因为 A$\rightarrow$B 成功率 95\%" \\
  \end{tabular}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{与传统 KG 的区别}
        ...
        \begin{tabular}{p{2cm}|p{3.5cm}}
          \textbf{维度} & \textbf{Context Graph} \\
          \hline
          设计目标 & AI 模型消费 \\
          核心要素 & 实体+关系+\textbf{决策事件} \\
          时间处理 & 双时态（有效时间+事务时间） \\
          溯源机制 & 核心组件 \\
        \end{tabular}
      \end{block}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{技术定位}
        \begin{itemize}
          \item \textbf{学术视角}：GraphRAG 的算法创新
          \item \textbf{产业视角}：决策痕迹捕获平台
          \item \textbf{Agent 视角}：结构化的经验记忆
        \end{itemize}
        \vspace{0.2cm}
        \small
        三个视角指向\textbf{同一技术栈}，切入点不同
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  \frametitle{GraphRAG vs Context Graph：概念辨析}

  \begin{alertblock}{核心论点}
    \textbf{GraphRAG 是检索技术，Context Graph 是企业架构愿景}。两者共享"图"结构，但解决的问题层次不同
  \end{alertblock}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{GraphRAG（技术层）}
        \small
        \begin{itemize}
          \item \textbf{目标}：改进 LLM 检索质量
          \item \textbf{输入}：静态文档语料库
          \item \textbf{输出}：增强的上下文
          \item \textbf{局限}：
          \begin{itemize}
            \scriptsize
            \item 离线批处理，非实时
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Context Graph（愿景层）}
        \small
        \begin{itemize}
          \item \textbf{目标}：捕获跨系统决策上下文
          \item \textbf{输入}：6-10 个业务系统实时状态
          \item \textbf{输出}：决策依据链
          \item \textbf{新增}：
          \begin{itemize}
            \scriptsize
  ...
  \begin{block}{关系类比}
    GraphRAG 之于 Context Graph $\approx$ TCP/IP 之于互联网：\textbf{关键技术组件}，但完整实现需要更多系统工程
  \end{block}
\end{frame}

\begin{frame}
  \frametitle{从 GraphRAG 到 Context Graph：五个关键 Gap}

  \small
  \begin{tabular}{p{0.8cm}|p{3.8cm}|p{4cm}|p{2.8cm}}
    \textbf{Gap} & \textbf{GraphRAG 现状} & \textbf{Context Graph 需求} & \textbf{差距性质} \\
    \hline
    \textbf{1} & 批量索引，小时/天级延迟 & 实时同步，秒级延迟 & 架构性 \\
    \hline
    \textbf{2} & 实体 + 关系（知识事实） & + 决策事件 + 政策节点 & 本体论 \\
    \hline
    \textbf{3} & 单一文档库 & 6-10 个异构系统 & 集成复杂度 \\
    \hline
    \textbf{4} & 静态快照 & 双时态（T\_valid + T\_tx） & 时态建模 \\
    \hline
    \textbf{5} & 返回相关文档片段 & 返回决策依据链 & 解释性 \\
  \end{tabular}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{alertblock}{关键判断}
        \footnotesize
        \begin{itemize}
          \item \textbf{GraphRAG 已成熟}：2024-2025 学术界充分验证
          \item \textbf{Context Graph 待解决}：需要架构层面重新设计
          \item \textbf{真正瓶颈}：57\% 公司数据未准备好支持 AI
        \end{itemize}
      \end{alertblock}
    \end{column}
...
\begin{frame}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.85\textwidth,height=0.72\textheight,keepaspectratio]{context_graph_example.jpg}
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[scale=0.9]
      ...
      % 第一层：问题
      \node[box, fill=red!20] (problem) at (0, 3) {Agent 需要记忆};

      % 第二层：两条路线
      \node[box, fill=blue!20] (graphrag) at (-3.5, 1.5) {图增强检索};
      \node[box, fill=green!20] (memory) at (3.5, 1.5) {Agent Memory};

      % 第三层：具体方法
      \node[box, fill=blue!10, font=\small] (grag1) at (-5.5, 0) {GraphRAG};
      \node[box, fill=blue!10, font=\small] (grag2) at (-3.5, 0) {CGR³};
      \node[box, fill=blue!10, font=\small] (grag3) at (-1.5, 0) {GoR};

      \node[box, fill=green!10, font=\small] (mem1) at (1.5, 0) {ExpeL};
      \node[box, fill=green!10, font=\small] (mem2) at (3.5, 0) {A-Mem};
      \node[box, fill=green!10, font=\small] (mem3) at (5.5, 0) {可训练图记忆};

      % 连线
      \draw[->, thick] (problem) -- (-1.75, 1.5) -- (graphrag);
      \draw[->, thick] (problem) -- (1.75, 1.5) -- (memory);
      \draw[->] (graphrag) -- (grag1);
      \draw[->] (graphrag) -- (grag2);
      \draw[->] (graphrag) -- (grag3);
      \draw[->] (memory) -- (mem1);
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{block}{Zettelkasten 方法}
        \begin{enumerate}
          \item 每个记忆单元生成结构化属性（关键词、标签、描述）
          \item 分析历史记忆，建立语义关联链接
          \item 新记忆可触发旧记忆的\textbf{属性更新}
          \item 记忆网络持续演化
        \end{enumerate}
      \end{block}
      ...
      \begin{block}{实验结果（NeurIPS 2025）}
        \begin{itemize}
          \item 复杂推理任务性能\textbf{翻倍}
          \item Token 消耗显著降低
          \item 6 个基础模型上均优于 SOTA
        \end{itemize}
      \end{block}
    \end{column}
...
\begin{frame}
  \frametitle{FlexRule 的批判：决策考古学陷阱 \cite{flexrule2024critique}}

  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{核心批评}
        单纯依赖历史轨迹 = 将\textbf{"先例误认为政策"}
      \end{alertblock}

      \begin{block}{"决策考古学"问题}
        \begin{itemize}
          \item 从历史归纳规则，而非明确意图
          \item 无法区分有效逻辑 vs 错误决策
          \item 只能说"统计相似"，不能说"应该"
        \end{itemize}
      \end{block}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{替代方案：决策中心法}
        \small
        \begin{enumerate}
          \item \textbf{意图层}：显式定义决策逻辑
          \item \textbf{流程层}：管理决策触发时机
          \item \textbf{输入层}：提供决策就绪数据
          \item \textbf{痕迹层}：事后捕获，仅用于审计
          \item \textbf{学习层}：显式优化，避免漂移
        \end{enumerate}
      \end{block}

      \begin{alertblock}{对研究的启示}
        Context Graph 不能\textbf{只}存储历史轨迹\\
        需要区分：\textbf{描述性}知识 vs \textbf{规范性}知识
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  \frametitle{现有方法的关键 Gap}

  \begin{block}{Gap 1：图结构设计缺乏共识}
    \begin{itemize}
      \item 节点是什么？Action / State / Observation / Strategy？
      \item 边表示什么？时序 / 因果 / 语义相似？
      \item 粒度如何选择？细粒度 vs 抽象
    \end{itemize}
  \end{block}

  \begin{block}{Gap 2：从"记录"到"学习"的鸿沟}
    \begin{itemize}
      \item 大多数工作停留在"存储-检索"范式
      \item 如何从失败经验中\textbf{主动学习}？
      \item 如何避免重复相同错误？
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Gap 3：领域特定应用的空白}
    \begin{itemize}
      \item 现有工作主要在通用 QA 任务上验证
      \item Coding Agent 场景的 Context Graph 研究\textbf{几乎空白}
      \item Bug 修复、漏洞检测等任务有独特的图结构需求
    \end{itemize}
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{block}{为什么选择 Bug 修复？}
        \begin{itemize}
          \item 目前\textbf{最主流、最有挑战性}的 Coding Agent 任务
          \item 有成熟的 benchmark (SWE-bench) 和 leaderboard
          \item 与漏洞检测相关：很多 issue 本身就是 bug/vulnerability
        \end{itemize}
      \end{block}

      \begin{block}{Context Graph 要提升的三个指标}
        \begin{enumerate}
          \item \textbf{成功率 (Resolved Rate)}
          \begin{itemize}
            \item 问题：Agent 经常在同类问题上重复失败
            \item CG 作用：记录失败模式，避免重蹈覆辙
          \end{itemize}
          \item \textbf{效率 (Token/Cost)}
          \begin{itemize}
            \item 问题：解决一个 issue 需要大量 token
            \item CG 作用：Trajectory Reduction，压缩历史
          \end{itemize}
          \item \textbf{持续学习 (Continual Learning)}
          \begin{itemize}
            \item 问题：每次任务从零开始，不会积累
            \item CG 作用：跨任务知识迁移
          \end{itemize}
        \end{enumerate}
      \end{block}
    ...
    \begin{column}{0.42\textwidth}
      \begin{alertblock}{关键问题}
        ...
        \begin{itemize}
          \item 为什么之前失败？\\
                \textcolor{gray}{\small 因为项目用 gradle 不用 maven}
          \item 正确流程是什么？\\
                \textcolor{gray}{\small 图上清晰展示}
          \item 哪条路径最可靠？\\
                \textcolor{gray}{\small 看边的成功率}
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}
  \frametitle{方向 2：漏洞检测（备选）}

  \begin{block}{Bug 修复 vs 漏洞检测：关键差异}
    \small
    \begin{tabular}{p{2cm}|p{4.5cm}|p{4.5cm}}
      \textbf{维度} & \textbf{Bug 修复 (SWE-bench)} & \textbf{漏洞检测} \\
      \hline
      任务起点 & 已知 Issue 描述 & 未知，需要主动发现 \\
      \hline
      目标 & 让测试通过 & 发现+定位+验证漏洞 \\
      \hline
      评估标准 & Resolved Rate & 检测率 + 误报率 + 定位精度 \\
      \hline
      知识依赖 & 项目代码逻辑 & 安全知识 (CWE/CVE/OWASP) \\
      \hline
      验证方式 & 单元测试通过 & PoC/Exploit 验证 \\
      \hline
      容错性 & 较高（可多次尝试） & 较低（误报代价高） \\
      \hline
      工具链 & 读/写文件、运行测试 & 静态分析、动态分析、Fuzzing \\
    \end{tabular}
  \end{block}

  \begin{alertblock}{为什么 Bug 修复优先？}
    \begin{itemize}
      \item 评测基准成熟（SWE-bench），便于验证 Context Graph 效果
      \item 漏洞检测相关研究\textbf{几乎空白}，可作为后续方向
    \end{itemize}
  \end{alertblock}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Agent Memory 系统}
        \small
        \begin{enumerate}
          \item \textbf{ExpeL} \cite{zhao2024expel} (AAAI 2024)
          \begin{itemize}
            \item 经验学习，洞察提取
          \end{itemize}
